# Stock Positions & Trade History

*Vyse's trading record - rebuilt from scratch May 5, 2026*

---

## Current Positions (as of May 11, 2026 PT)

| Symbol | Shares | Entry | Current | Notes |
|--------|--------|-------|---------|-------|
| **AMC** | 17 | $1.44 | Day-to-Day | Entered May 11 |
| **DOGE** | 7.16 | $0.1118 | Crypto | $0.80 position |
| **QQQ** | 0.021 | $708.78 | Piggybank | Long-term hold |
| **WLFI** | 14.5 | $0.06873 | Crypto | Weekend only |

---

## Long-Term Piggybank (Separate from SMC)

| Symbol | Shares | Entry | Type | Thesis |
|--------|--------|-------|------|--------|
| QQQ | 0.021 | $708.78 | Piggybank | AI trend, Nasdaq-100, years hold |

**DCA Rule:** 5% of all trade profits → QQQ piggybank
- Profits from SMC trades go here
- Fractional shares = any amount works
- Set and forget

## Capital Summary

| Metric | Amount |
|--------|--------|
| **Total Capital** | **$55.46** |
| **Buying Power** | **$38.72** |
| **Invested** | **$16.64** |

---

## Trade History

| Date | Action | Symbol | Shares | Price | P/L | Notes |
|------|--------|--------|--------|-------|-------|-------|
| 2026-05-05 | BUY | NVDA | 0.050426 | $198.31 | - | David's pick - AI/GPU play |
| 2026-05-05 | BUY | WGS | 1 | $35.94 | - | David's independent research - bull case |
| 2026-05-07 | SELL | WGS | 1 | $38.36 | +$2.42 (+6.7%) | Exit at target |
| 2026-05-07 | BUY | NIO | 1 | $5.94 | - | SMC: Liquidity→Sweep→CHOCH→FVG |
| 2026-05-08 | SELL | NVDA | 0.050426 | $215.00 | +$0.92 (+9.3%) | Exit |
| 2026-05-08 | SELL | NIO | 1 | $5.87 | -$0.07 (-1.2%) | Exit (ADR rule) |
| 2026-05-08 | BUY | QQQ | 0.021 | $708.78 | - | Piggybank from profits |
| 2026-05-08 | SELL | DOGE | 1.00 | $0.11 | -$0.01 | Sold free DOGE |
| 2026-05-08 | BUY | DOGE | 7.16 | $0.1118 | - | Rebought with $0.80 |
| 2026-05-09 | BUY | WLFI | 14.5 | $0.06873 | - | Crypto weekend |

---

## Trading Rules - SMC METHOD

### The 6 Steps (Complete Guide)
1. **Find Liquidity** — Equal lows, obvious support, prior swing low, clean level everyone sees
2. **Wait for Sweep** — Price dips below liquidity → rejects (liquidity grab)
3. **Wait for CHOCH** — Strong break in structure, invalidates prior trend, clear momentum shift
4. **Wait for FVG** — Price retraces into Fair Value Gap (imbalance) = ENTRY ZONE
5. **Draw Fib** — From liquidity grab low → CHOCH high
6. **TP/SL** — TP at -2, SL below liquidity

### SMC Checklist (Must Have All 6)
- [ ] Liquidity taken (sweep + rejection)
- [ ] Clear CHOCH (break above prior high)
- [ ] Retrace into FVG
- [ ] Fib drawn (liquidity → CHOCH)
- [ ] TP at -2
- [ ] SL below liquidity

**If all 6 line up = SETUP VALID**

### Fib Settings (EXTENSIONS)
- Use: 0, -1, -2 (REMOVE standard retracement levels)
- 0 = Entry point (at CHOCH)
- -1 = First target (1x range expansion)
- -2 = Second target (2x range expansion)

### Position Rules (SMC)
- **Take Profit:** -2 level (2x expansion)
- **Stop Loss:** Just below liquidity grab (with buffer)
- **Max Position:** 25% of capital per stock
- **Max Positions:** 5 stocks at a time

### Why This Works
- Liquidity gets taken → Structure shifts → Price retraces to imbalance → Expansion continues
- No early profit-taking (let expansion complete)
- No emotional exits (math-based targets)
- Consistent R:R

### Volume Checklist
- [ ] Volume trending with trend (confirms)
- [ ] Big candle + low volume = potential reversal
- [ ] Volume spike at key level = institutional activity
- [ ] Divergence (lower highs + higher volume) = reversal likely

### Scanner v4 Criteria
- Near swing low (<45% of range from low)
- Bounced (price above swing low)
- Volume confirmation (>0.5x avg)
- Status: NEAR LIQUIDITY, LOW RANGE, MID RANGE, NEAR HIGH

### Legacy Rules (if no chart)
- **Stop Loss:** -7% from entry
- **Take Profit:** +12% from entry

---

*Updated May 7, 2026 - SMC method fully documented*