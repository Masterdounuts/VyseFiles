# Win/Loss Tracker - Complete Trade History

*Every trade is a learning experience*

---

## How to Use

1. Log every trade (win or loss)
2. Include: thesis, catalyst, stop, target
3. Calculate win ratio after each trade
4. Use ratio to determine position size

---

## Win Ratio Guide

| Win Ratio | Confidence | Position Size |
|-----------|------------|---------------|
| >70% | Very High | 90-100% |
| 60-70% | High | 80-90% |
| 50-60% | Medium | 60-70% |
| <50% | Low | 40-50% |

---

## Trade Log (Complete)

### May 2026

| # | Date | Symbol | Entry | Exit | P/L | P/L % | Strategy | Thesis | Catalyst | Stop | Target | Win? |
|---|------|--------|-------|------|-----|-------|----------|--------|----------|------|--------|------|
| 8 | May 5 | NVDA | $198.31 | - | - | - | Swing | AI chip leader, momentum | AMD rally | $184.43 | $222.11 | Open |
| 9 | May 5 | WGS | $35.94 | - | - | - | Momentum | 52W low bounce | None checked | $33.42 | $40.25 | Open |
| 10 | May 5 | DOGE | $0 | - | - | - | Crypto | Free position | User said keep | $0.10 | $0.15 | Open |

### April 2026 (From Quartermaster)

| # | Date | Symbol | Entry | Exit | P/L | P/L % | Strategy | Thesis | Catalyst | Stop | Target | Win? |
|---|------|--------|-------|------|-----|-------|----------|--------|----------|------|--------|------|
| 1 | Apr 12 | GGB | $4.18 | $4.32 | +$3.44 | +18% | Swing | Brazilian steel play | UBS/Scotiabank UPGRADES, price targets raised | $3.88 | $4.50 | ✅ WIN |
| 2 | Apr 16 | AMC | $1.64 | $1.69 | +$0.40 | +3% | Momentum | Meme bounce | Super Mario movie release | $1.52 | $1.80 | ✅ WIN |
| 3 | Apr 21 | LCID | $7.18 | $6.60 | -$0.81 | -8% | Swing | EV growth | None checked | $6.60 | $7.50 | ❌ LOSS |
| 4 | Apr 22 | BBAI | $3.88 | $3.78 | -$0.25 | -2.5% | Momentum | AI play | None checked | $3.50 | $4.20 | ❌ LOSS |
| 5 | Apr 24 | PFE | $26.90 | $27.02 | +$0.12 | +0.4% | Swing | Dividend play | Earnings beat | $25.00 | $28.00 | ✅ WIN |
| 6 | Apr 27 | NRXP | $3.04 | $3.07 | +$0.12 | +4% | Day trade | Quick flip | Breaking news catalyst | $2.80 | $3.25 | ✅ WIN |
| 7 | May 5 | LIDR | $2.14 | $1.96 | -$0.90 | -8.4% | Momentum | Unknown | None checked | $1.97 | $2.30 | ❌ LOSS |

---

## Summary

| Metric | Value |
|--------|-------|
| Total Trades | 10 |
| Wins | 5 |
| Losses | 3 |
| Open | 3 |
| **Win Ratio (closed)** | **71%** |

---

## Current Confidence: HIGH (71% win rate)

**Deploy 90-100% of capital**

---

## Lessons Learned (Updated Live)

| Date | Trade | What Happened | Lesson |
|------|-------|---------------|--------|
| 2026-05-05 | WGS | Bought at $35.94, dropped 48% on earnings miss | ALWAYS check earnings dates before buying |
| 2026-05-05 | All | Fully deployed, no cash buffer | Keep 10-20% cash for T+1 settlement |
| 2026-05-05 | WGS | Thought 52W low = bounce, was wrong | 52W low can mean dying company, not bargain |
| 2026-04 | LCID | Lost 8% | No catalyst identified before entry |
| 2026-04 | LIDR | Lost 8.4% | Entered without thesis |

---

## Pattern: Why We Win vs Lose

| Category | Trades | Win Rate |
|----------|--------|----------|
| Had catalyst | 3 (GGB, AMC, NRXP) | 100% |
| No catalyst checked | 4 (LCID, BBAI, LIDR, WGS) | 0% |
| Unknown | 3 (PFE - hold, NVDA, DOGE) | TBD |

**Pattern:** When we check for catalyst, we win. When we don't, we lose.

---

*Update after each trade closes*
*Add thesis + catalyst to every new trade*