# Knowledge Base Index

**Fast retrieval:** Static knowledge → `kb/`, Daily logs → `memory/`

---

## 📈 Stocks (`kb/stocks/`)

### Core
- [[kb/stocks/trade-log.md|Trade Log]] - Executed trades
- [[kb/stocks/rules.md|Trading Rules]] - Strategy & levels (move from memory/trading_rules.md if exists)

### Research (`kb/stocks/research/`)
- [[kb/stocks/research/morning-research.md|Morning Research]]
- [[kb/stocks/research/weekly-research.md|Weekly Research]]
- [[kb/stocks/research/day-trade-research.md|Day Trade Research]]
- [[kb/stocks/research/volatile-stocks.md|Volatile Stocks]]
- [[kb/stocks/research/research.md|Research Index]]
- *Archived:* alert-status, day-trading-options, income-strategy, model-performance, news-alert, robinhood-android-app

---

## ⚙️ System (`kb/system/`)

- [[kb/system/BOOTSTRAP.md|Wake-up Protocol]]
- [[kb/system/FIXES.md|Known Issues]]
- [[kb/system/INTERRUPT_RECOVERY.md|Recovery]]

---

## 👤 Personal

- [[../IDENTITY.md|Identity]] - Name, vibe
- [[../SOUL.md|Soul]] - Core principles
- [[../USER.md|User]] - Your prefs

*(Files in workspace root)

---

## 📚 Reference (`kb/reference/`)

*(Empty - add reference docs here)*

---

*KB = static knowledge, memory = time-series*